...
Keep `--placement split`. The viewer can also open as an overlay or in a floating, session-modal
popup, but a popup takes the whole session over and blocks every other surface until the user exits
the viewer — the wrong default for an open you initiated. If the user explicitly asks for one of
those placements, also pass the matching marker — `--env HERDR_FILE_VIEWER_PLACEMENT=popup` or
`=overlay`: the marker is how the viewer knows it does not own a normal tiled pane, so its `Z`
full-screen key stays in-pane. Without it, in a popup `Z` asks herdr to zoom the pane underneath
(someone else's), and in an overlay releasing that zoom strips the overlay's cover and flattens it
into a plain split.
